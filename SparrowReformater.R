...
if (Upload) {
  ...
  for (k in 1:length(levels(Output$GUESS.SAMP))) {
    ...
    request <- jsonlite::toJSON(list(filename = paste(BaseFile, k),
                    data = Session),
                    auto_unbox = TRUE)
                # instrument_session <- list(data = list(name = BaseFile))
                #
                #   print(toJSON(instrument_session, auto_unbox = TRUE, pretty = TRUE))
                #
                #     response <-
                #       PUT(url = "http://backend:5000/api/v2/import-data/instrument_session",
                #           body = instrument_session,
    ...
    if (Upload) {
      response <-
        PUT(url = "http://backend:5000/api/v2/import-data/models/session",
            body = request,
