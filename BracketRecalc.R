BracketRecalc <- function(InputFile, IsotopeMethod) {
  ...
  Output$UNIQUEGRP <-
    as.factor(paste(Output$MATERIAL, Output$GROUPNUM))
  levels(Output$UNIQUEGRP)[levels(Output$UNIQUEGRP) == "NA NA"] <-
    NA
  Output <- Output[order(Output$GROUPNUM),]
  AllGroups <- unique(as.character(Output$UNIQUEGRP))
  
  Samples <-
    unique(Output$UNIQUEGRP[Output$MATERIAL == "Sample" &
                              !is.na(Output$MATERIAL)])
  ...
  for (i in 1:length(Samples)) {
    ...
    ReplaceLogic <-
      Output$UNIQUEGRP == Samples[i] &
      is.na(Output$UNIQUEGRP) == FALSE
    
    SelectLogic <-
      Output$UNIQUEGRP == StartGroup &
      !is.na(Output$UNIQUEGRP) |
      Output$UNIQUEGRP == EndGroup & !is.na(Output$UNIQUEGRP)
    
    
    if (length(Output$BRACKET2SD[ReplaceLogic]) >= 1 &&
        length(Output$BRACKET2SD[SelectLogic]) >= 1) {
      if (length(unique(Output$RegexSTD[SelectLogic])) == 1) {
        RunStd <-
          as.numeric(Standards[Standards$StdName == unique(Output$RegexSTD[SelectLogic]), Isotope])
        
        Output$BRACKET2SD[ReplaceLogic] <-
          2 * sd(Output[SelectLogic, 7])
        
        MeanIso <-
          round(mean(Output[SelectLogic, 7], na.rm = TRUE), digits = 8)
        
        BracketBias <-
          (((1 + MeanIso / 1000) / (1 + RunStd / 1000)) -
             1) * 1000
        
        Output$STDiso[ReplaceLogic] <-
          (((1 + Output[ReplaceLogic, 7] / 1000) / (1 + BracketBias / 1000)) - 1) *
          1000
        
        Output$REL_YIELD[ReplaceLogic] <-
          Output$Yield[ReplaceLogic] / mean(Output$Yield[SelectLogic], na.rm = TRUE)
        
        Output$REL_Hyd[ReplaceLogic] <-
          Output[ReplaceLogic, "Hyd"] - mean(Output[SelectLogic, "Hyd"], na.rm =
                                               TRUE)
      }
    }
  }
